...
for provider in provider_ids:
    ...
    providers_dict[provider]['First Name'] = df[df['Provider ID'] ==
                                                provider]['Provider Waiting Room Status Provider First Name'].iloc[0]
    providers_dict[provider]['Last Name'] = df[df['Provider ID'] ==
                                               provider]['Provider Waiting Room Status Provider Last Name'].iloc[0]
    providers_dict[provider]['Provider ID'] = provider
    providers_dict[provider]['NPI'] = df[df['Provider ID'] ==
                                         provider]['Provider Waiting Room Status Provider NPI'].iloc[0]
...
for provider in provider_ids:
    ...
    for i in range(0, len(dates)):
        ...
        for index, event in date_df.iterrows():
            print('loop date: ' + str(loop_date) + ' ' + 'index: ' + str(index) + ' ' + 'in_flag: ' + str(in_flag) + ' ' +
                  str(event['Provider Waiting Room Status EST Event Time']) + ' ' + str(event['Provider Waiting Room Status Status']))
            quit_flag = 0
...
for provider in providers_dict:
    logged_in_dates = list(
        providers_dict[provider]['Logged In Deltas by Date'].keys())
    available_dates = list(
        providers_dict[provider]['Available Deltas by Date'].keys())
    ...
    for date in dates:
        ...
        try:
            output[index]['Business Hours'] = str(sum(
                providers_dict[provider]['Business Deltas by Date'][date], datetime.timedelta()))[-9:]
        except:
            output[index]['Business Hours'] = ''
        try:
            output[index]['Moonlight Hours'] = str(sum(
                providers_dict[provider]['Moonlight Deltas by Date'][date], datetime.timedelta()))[-9:]
        except:
...
df_final.iloc[:, 0] = pd.to_datetime(
    df_final.iloc[:, 0], errors='coerce').dt.date
